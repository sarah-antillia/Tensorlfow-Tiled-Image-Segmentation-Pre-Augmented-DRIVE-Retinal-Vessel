...
class BaseImageMaskDataset:
  ...
  def __init__(self, config_file):
    ...
    #2024/12/14
    #contrast_ajuster = True
    #contrast_alpha   = 1.5
    #contrast_beta    = 40

    self.contrast_adjuster = self.config.get(ConfigParser.IMAGE, "contrast_adjuster", dvalue=False)
  
    print("---- constrast_adjuster {}".format(self.contrast_adjuster ))
   
    self.contrast_alpha    = self.config.get(ConfigParser.IMAGE, "contrast_alpah",    dvalue=1.5)
    self.contrast_beta     = self.config.get(ConfigParser.IMAGE, "contrast_best",     dvalue=40)

    #  2025/01/13
    self.reverse     = self.config.get(ConfigParser.IMAGE, "reverse",     dvalue=False)
  ...
  # 2024/12/14
  def contrast_adjust(self, img, alpha, beta):
